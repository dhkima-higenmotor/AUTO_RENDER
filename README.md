...
* **`EXPLODE`** 버튼을 클릭하면 다음과 같은 조립분해도 애니메이션 렌더링 프로세스가 진행됩니다:
  1. **분해 방향 축 선택 팝업**: 분해 애니메이션이 진행될 기준 축(X, Y, Z축 중 선택, 기본값 Y축)을 선택할 수 있는 팝업창이 나타납니다.
  ...
  6. 지정한 분해 축 및 총 재생 시간을 반영한 `explode.py` 애니메이션 스크립트 소스코드를 Blender의 스크립트 창(Text block)에 복사하고 실행하여 순차 분해 애니메이션 타임라인을 새로 생성합니다.
